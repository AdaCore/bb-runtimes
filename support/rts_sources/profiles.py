...
class RTSProfiles(object):
    ...
    def check_deps(self, scenarios):
        while True:
            modified = False
            for d, content in rts_sources.items():
                matches = False
                ...
                if 'conditions' not in content:
                    ...
                    rule = Rule(content['conditions'], rts_scenarios)
                    if rule.matches(scenarios):
                        matches = True
                if matches:
                    dep = Rule(content['requires'], rts_scenarios)
    ...
    def zfp_scenarios(self, mem_routines, math_lib, profile='zfp'):
    ...
    def full_scenarios(self, mem_routines, math_lib, small_mem):
        ...
        ret = self.sfp_scenarios(
            mem_routines, math_lib, small_mem, 'ravenscar-full')
