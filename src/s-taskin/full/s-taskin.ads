...
--  This is a Ravenscar version of this package, extended with support for
...
--  The Initialize procedure is called by System.Init.Install_Handler prior to
--  the elaboration procs, and we must enforce that nothing in the latter might
--  clobber what the early inits did. This is very tricky in presence of
--  elaboration code for this unit, so we make sure that we don't have any and
--  stick a few Suppress_Initialization pragmas to honor the restriction. The
--  code relies on a few components of globals being implicitly initialized
...
package System.Tasking is
   ...
   type Task_States is
     (Unactivated,
      --  Task has been created but has not been activated. It cannot be
      --  executing.
   ...
   --  Used to represent protected procedures to be executed when a task
   --  terminates.
   ...
   --  Above pragma should be documented ???
