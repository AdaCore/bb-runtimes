...
begin
   ...
   while PMC_Periph.SR.MOSCXTS = 0 loop
      null;
   end loop;
   ...
   while PMC_Periph.SR.MOSCSELS = 0 loop
      null;
   end loop;
   ...
   while PMC_Periph.SR.LOCKA = 0 loop
      null;
   end loop;
   ...
   PMC_Periph.MCKR.PRES := CLK_1;
   while PMC_Periph.SR.MCKRDY = 0 loop
      null;
   end loop;

   PMC_Periph.MCKR.MDIV := PMC.PCK_DIV2;

   while PMC_Periph.SR.MCKRDY = 0 loop
      null;
   end loop;

   PMC_Periph.MCKR.CSS := PMC.PLLA_CLK;
   while PMC_Periph.SR.MCKRDY = 0 loop
      null;
   end loop;
