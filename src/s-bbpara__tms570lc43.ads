...
package System.BB.Parameters is
   ...
   subtype Interrupt_Range is Natural range 0 .. 127;
   --  Number of interrupts supported by the VIC. For the TMS570 interrupts,
   --  we really consider the 127 usable interrupt channels even though not all
   --  are used. The run time assumes the interrupt source to interrupt
