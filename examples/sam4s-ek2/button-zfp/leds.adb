...
procedure Leds is
   ...
   Led1_Pin : constant := 2 ** 19; -- PA19
   Led2_Pin : constant := 2 ** 20; -- PA20

   But2_Pin : constant := 2 ** 12; -- PC12

   Slow : Boolean := False;

   procedure Wait is
   begin
      for J in False .. Slow loop
...
begin
   ...
   PMC.PCER0 := 2 ** PIOA_ID or 2 ** PIOC_ID;
   ...
   PIOA.PER := Led1_Pin + Led2_Pin;
   PIOA.OER := Led1_Pin + Led2_Pin;
   PIOA.CODR := Led1_Pin + Led2_Pin;
   PIOA.MDDR := Led1_Pin + Led2_Pin;

   loop
      --  Off

      if (PIOC.PDSR and But2_Pin) = 0 then
         Slow := not Slow;
      end if;
      PIOA.SODR := Led2_Pin;
      PIOA.CODR := Led1_Pin;
