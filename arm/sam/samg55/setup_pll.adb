...
procedure Setup_Pll is
   ...
   procedure Enable_Oscillator (Osc : Oscillators);
   --  Enables a specific oscillator
   ...
   procedure Enable_Oscillator (Osc : Oscillators)
   is
      MR : Interfaces.SAM.SYSC.SUPC_MR_Register;
      CR : Interfaces.SAM.SYSC.SUPC_CR_Register;
      CKGR_MOR : Interfaces.SAM.PMC.CKGR_MOR_Register;

   begin
      case Osc is
         when Internal_32k_RC =>
            --  Nothing to do here: Enabled by default
            null;

         when External_32k_XTAL | External_32k_BYPASS =>
            --  Set the SCK to the external 32k crystal
            if Osc = External_32k_BYPASS then
               MR := SUPC_Periph.MR;
               MR.KEY := Passwd;
               MR.OSCBYPASS := Bypass;
               SUPC_Periph.MR := MR;
            ...
            CR := SUPC_Periph.CR;
            CR.KEY := Passwd;
            CR.XTALSEL := Crystal_Sel;
            SUPC_Periph.CR := CR;

            --  wait for SCK ready
            while SUPC_Periph.SR.OSCSEL /= Cryst
              and then not PMC_Periph.PMC_SR.OSCSELS
            loop
               null;
            end loop;

         when Internal_8M_RC | Internal_16M_RC | Internal_24M_RC =>
            --  This sets the MCK source to the internal oscillator.

            --  Enable the internal oscillator
            CKGR_MOR := PMC_Periph.CKGR_MOR;
            CKGR_MOR.KEY := Passwd;
            CKGR_MOR.MOSCRCEN := True;
            PMC_Periph.CKGR_MOR := CKGR_MOR;

            while not PMC_Periph.PMC_SR.MOSCRCS loop
               null;
            end loop;
            ...
            --  Use the Fast RC as main clock input
            CKGR_MOR := PMC_Periph.CKGR_MOR;
            CKGR_MOR.KEY := Passwd;
            CKGR_MOR.MOSCSEL := False; --  True for the external crystal
            PMC_Periph.CKGR_MOR := CKGR_MOR;

            --  Wait for the main clock status flag
            while not PMC_Periph.PMC_SR.MOSCSELS loop
               null;
            end loop;

         when External_XTAL | External_BYPASS =>
            --  This sets the MCK source to the external oscillator.

            --  Enable the main xtal oscillator
            CKGR_MOR := PMC_Periph.CKGR_MOR;
            CKGR_MOR.KEY := Passwd;

            if Osc = External_BYPASS then
               CKGR_MOR.MOSCXTEN := False;
               CKGR_MOR.MOSCXTBY := True;
               CKGR_MOR.MOSCSEL := True;
            else
               CKGR_MOR.MOSCXTEN := True;
               CKGR_MOR.MOSCXTBY := False;
               CKGR_MOR.MOSCXTST := External_Oscillator_Startup_Time;
            end if;
            ...
            --  Use the XTAL as main clock input
            CKGR_MOR := PMC_Periph.CKGR_MOR;
            CKGR_MOR.KEY := Passwd;
            CKGR_MOR.MOSCSEL := True;
            PMC_Periph.CKGR_MOR := CKGR_MOR;

            --  Wait for the MOSCSELS flag
            while not PMC_Periph.PMC_SR.MOSCSELS loop
               null;
            end loop;
      end case;
   end Enable_Oscillator;
...
begin
   ...
   EFC_Periph.FMR.FWS := 5;

   --  18.17 Programming Sequence

   --  1-5: Configure the oscillator and select it as either Slow_Clk source
   --  or Main_Clk source
   pragma Warnings (Off, "*condition is always*");
   if Master_Source = PLLA then
      pragma Assert (PLLA_Enable);
      Osc := PLLA_Source;
   elsif Master_Source = PLLB then
      pragma Assert (PLLB_Enable);
      Osc := PLLB_Source;
   else
      Osc := Master_Source;
   end if;
   ...
   --  Enable Osc and set it as source for the slow or main clock

   case Osc is
      when Internal_32k_RC | External_32k_XTAL | External_32k_BYPASS =>
         --  Enable the oscillator
         Enable_Oscillator (Osc);

      when Internal_8M_RC | Internal_16M_RC | Internal_24M_RC |
           External_XTAL | External_BYPASS =>
         --  Enable the oscillator
         Enable_Oscillator (Osc);

         if Osc in External_XTAL .. External_BYPASS then
            --  5. Check the Main clock frequency
            loop
               exit when PMC_Periph.CKGR_MCFR.MAINFRDY;
            end loop;

            --  MAINF contains the number of Main ticks between two slow
            --  clock ticks. If 0, this means that the external main clock is
            --  not available.
   ...
   PMC_Periph.CKGR_PLLAR.PLLAEN := 0;
   PMC_Periph.CKGR_PLLBR.PLLBEN := 0;
   ...
   if PLLB_Enable then
      PMC_Periph.CKGR_PLLBR :=
        (ZERO   => False,
         MULB   => PLLB_Mul,
         PLLBEN => 1,
         others => <>);

      while not PMC_Periph.PMC_SR.LOCKB loop
         null;
      end loop;
   end if;
   ...
   PMC_Periph.PMC_MCKR.PRES := Master_Prescaler;

   while not PMC_Periph.PMC_SR.MCKRDY loop
      null;
   end loop;

   case Master_Source is
      when Internal_32k_RC | External_32k_XTAL | External_32k_BYPASS =>
         --  Switch the system clock to the slow clock
         PMC_Periph.PMC_MCKR.CSS := Slow_Clk;

      when Internal_8M_RC | Internal_16M_RC | Internal_24M_RC |
           External_XTAL | External_BYPASS =>
         --  Switch the system clock to the main clock
      ...
      when PLLB =>
         PMC_Periph.PMC_MCKR.CSS := Pllb_Clk;
