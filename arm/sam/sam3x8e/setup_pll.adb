...
begin
   ...
   --  to be raised if the associated interrupt to MOSCXTS has been
   --  enabled in the PMC_IER register.

   CKGR_MOR := PMC_Periph.CKGR_MOR;
   --  The Key field needs to be set to 0x37 to enable write to the
   --  CKGR_MOR register
   CKGR_MOR.KEY := CKGR_MOR_KEY_VALUE;
   CKGR_MOR.MOSCXTEN := 1;  --  Enable main crystal oscillator;
   CKGR_MOR.MOSCXTST := 62; --  Number of slow clock cycles for startup
   CKGR_MOR.MOSCRCF  := Val_12_Mhz;  --  Select RC oscillator fequency of 12MHz
   PMC_Periph.CKGR_MOR := CKGR_MOR;

   --  Wait until the xtal stabilize
   while PMC_Periph.PMC_SR.MOSCXTS = 16#0# loop
      null;
   end loop;
   ...
   CKGR_MOR.KEY := CKGR_MOR_KEY_VALUE;
   CKGR_MOR.MOSCXTEN := 1;  --  Enable main crystal oscillator
   CKGR_MOR.MOSCSEL  := 1;  --  Select main crystal oscillator
   PMC_Periph.CKGR_MOR := CKGR_MOR;
   ...
   while PMC_Periph.PMC_SR.LOCKA = 16#0# loop
      null;
   end loop;
   ...
   while PMC_Periph.PMC_SR.MCKRDY = 16#0# loop
      null;
   end loop;
